...
* Date
*
* Implementation of Systolic Montgomery Multiplication, Addition
, and Subtraction.
6 * Systolic Montgomery Multiplication : B. Dixon, A.K. Lenstra
7 *
8 */
...
extern __device__ unsigned int atomicOr(unsigned int *address,
                                        unsigned int val);
// Bit Conversions
...
Do not use this, it is possible where your neighbors will not
get the shifts
...
void validate(Integer *A, Integer *B, Integer *M, Integer *R, int r, int d,
	      Integer *Results, std::ostream);
#endif
...
#define lowF(f) __longlong_as_double( __double_as_longlong( f )
& (0x7FFFFF) )

// We only need the bottom 23 bits
#define hiF(f) __longlong_as_double( (__double_as_longlong( f )
>> 23) & (0x7FFFFF) )
#endif
...
// Works!
__device__ void d_mul_concurrent_sync(P_BASE __restrict A, P_BASE __restrict B,
				      P_BASE __restrict R, BASE n,
...
// Works!
__device__ void d_mul_concurrent(P_BASE __restrict A, P_BASE __restrict B,
				 P_BASE __restrict R, BASE n, unsigned int r,
				 unsigned int d) {
...
__global__ void with_sync(P_BASE A, P_BASE B, __restrict M, BASE r, BASE d,
			  unsigned int muls, P_BASE Results) {

	// Calculate when to stop
	P_BASE max = A + muls * r;

	// Calculate starting point for this block
	// A : [blockDim.x] [blockDim.x] [blockDim.x]
	// BDX : 0
	A = A + BDX * DELTA;
	B = B + BDX * DELTA;
	...
	// Do work
	while ((A + IDX) < max) {
		d_mul_concurrent_sync( A , B , Results , n ,
...
__global__ void no_sync(P_BASE A, P_BASE B, C_P_BASE __restrict M, BASE r,
			BASE d, unsigned int muls, P_BASE Results) {
...
void StartCounter(std::ostream &out) {
	LARGE_INTEGER li;
...
void validate(C_P_BASE _A, C_P_BASE _B, C_P_BASE _M, C_P_BASE Results,
	      unsigned int r, unsigned int d, int size, std::ostream &out) {
	Integer M(_M, r);
	Integer R("1");
	R <<= (r * BITLEN);
	R %= M;
...
extern "C" void run_kernel(unsigned int do_verify, int blocks, int threads,
			   C_P_BASE data, unsigned int data_length, C_P_BASE N,
			   unsigned int r, unsigned int d, unsigned int runs,
			   unsigned muls_per_blocks, std::ostream &out) {
	...
	errorTestMsg("\tMAL : N", cudaMalloc((void **)&dev_N, sizeof(BASE) * r),
		    std::cerr);
	if (dev_R == 0) {
	...
	// Copy data over
	errorTestMsg("\tMCPY : A",
		    cudaMemcpy(dev_A, A, gpu_bytes, cudaMemcpyHostToDevice),
		    std::cerr);
	errorTestMsg("\tMCPY : B",
		    cudaMemcpy(dev_B, B, gpu_bytes, cudaMemcpyHostToDevice),
		    std::cerr);
	errorTestMsg("\tMCPY : N",
		    cudaMemcpy(dev_N, N, sizeof(BASE) * r, cudaMemcpyHostToDevice),
		    std::cerr);
	...
	for(int i = 0; i < runs; i++) {
		...
		if (threads > 32 && (32 % r) != 0) {
			with_sync<<<blocks, threads, total>>>(dev_A, dev_B, dev_N, r, d,
							multiplications, dev_R);
		} else {
			no_sync<<<blocks, threads, total>>>(dev_A, dev_B, dev_N, r, d,
						    multiplications, dev_R);
		}
